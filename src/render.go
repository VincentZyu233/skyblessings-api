...
func drawColoredBackground(dc *gg.Context, result *BlessingResult) error {
	...
	if err != nil {
		return fmt.Errorf("打开遮罩文件失败: %w", err)
	}
	defer maskFile.Close()

	maskImg, _, err := image.Decode(maskFile)
	if err != nil {
		return fmt.Errorf("解码遮罩图片失败: %w", err)
	}

	// 创建一个纯色的源
	bgColor := parseColor(result.ColorHex)
	src := image.NewUniform(bgColor)

	// 使用 DrawMask 进行高质量的遮罩合成
	// 这会利用遮罩的 alpha 通道进行平滑混合，消除锯齿
	draw.DrawMask(dc.Image().(draw.Image), dc.Image().Bounds(), src, image.Point{}, maskImg, image.Point{}, draw.Over)
...
func drawTextImage(dc *gg.Context, result *BlessingResult) error {
	...
	// 放在画布水平中心偏左的位置
	x := float64(config.Image.Width)*0.35 - float64(textImg.Bounds().Dx())/2
	y := float64(config.Image.Height)/2 - float64(textImg.Bounds().Dy())/2
